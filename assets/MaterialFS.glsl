...
uniform mat4 unf25;
const float cons26 = float(0.005000);
const vec4 cons36 = vec4(0.000000, 0.000000, 0.000000, 0.005000);
uniform float unf47;
uniform float unf50;
uniform vec3 unf56;
...
void main()
{
	...
	set_rgb_zero(tmp7);
	set_rgb_zero(tmp8);
	lamp_visibility_sun_hemi(unf9, tmp10, tmp11, tmp12);
	shade_inp(tmp4, tmp10, tmp15);
	shade_visifac(tmp15, tmp12, unf18, tmp19);
	shade_mul_value_v3(unf20, unf21, tmp22);
	test_shadowbuf(varposition, samp0, unf25, cons26, tmp15, tmp28);
	shade_mul_value(tmp19, vec4(tmp22, 1.0), tmp31);
	mtex_value_invert(tmp28, tmp33);
	mix_mult(tmp33, tmp31, cons36, tmp37);
	mtex_value_invert(tmp33, tmp39);
	shade_madd(vec4(tmp7, 1.0), tmp37, vec4(tmp2, 1.0), tmp43);
	shade_cooktorr_spec(tmp4, tmp10, tmp6, unf47, tmp48);
	shade_spec_t(tmp39, unf50, tmp12, tmp48, tmp53);
	shade_add_spec(tmp53, tmp22, unf56, tmp57);
